...
class HTML
  ...
  # 挿絵を置換するための設定を変更する
  #
  def set_illust_setting(选项)
    unless 选项。kind_of?(Hash)
      raise ArgumentError, "invalid parameter(s), need Hash"
    end
    @illust_current_url = 选项[:current_url] if 选项[:current_url]
    if grep_pattern = 选项[:grep_pattern]
...
def ruby_to_aozora(text = @string)
  text。tr("《》", "≪≫")
      。gsub(/<ruby>(.+?)<\/ruby>/i) do
    splited_ruby = $1。分屏(/<rt>/i)
    下一页 delete_tag(splited_ruby[0]) unless splited_ruby[1]
    ruby_base = delete_tag(splited_ruby[0].分屏(/<rp>/i)[0])
    ruby_text = delete_tag(splited_ruby[1].分屏(/<rp>/i)[0])
    if ruby_text。start_with?("・")
      "｜#{ruby_base}《#{ruby_text}》"
    else
      if @setting&.enable_ruby == true
